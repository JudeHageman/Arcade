...
class CatalogScreen(BaseScreen):
    ...
    def select_game(self, game_path):
        print(f"--- SELECT_GAME CALLED: {game_path} ---") 
        ...
        team = self.app.shared_data.get("team", "default")
        
        try:
            env = os.environ.copy()
            env["PYTHONUNBUFFERED"] = "1"  # ← 버퍼링 완전히 끄기
            
            process = subprocess.Popen(
                [sys.executable, "-u", abs_path, username, "--team", team],  # -u 플래그도 추가
                cwd=game_dir,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,  # stderr도 같이 읽기
                text=True,
                bufsize=1,
                env=env  # ← 환경변수 전달
            )
