...
ENEMY_SPAWN_DATA = [
    {
        "name": "Silly Crab",
        "sprite_name": "silly_crab",
        "spawn": (27, 24), 
        "waypoints": [(27, 24), (30, 24)], 
        "patrol_type": "back_and_forth", 
        "speed": 0.5, 
        "description": "Sparkling Shore - Uses [Sand Pinch]",
        "health": 50,  "exp": 150, "attack_damage": 5,
        "notice_radius": 120, "attack_radius": 50,
    },
    {
        "name": "Mischievous Monkey",
        "sprite_name": "mischievous_monkey",
        "spawn": (44, 11),  
        "waypoints": [(44, 11), (49, 11), (49, 16), (44, 16)], 
        "patrol_type": "circular", 
        "speed": 1.0, 
        "description": "Emerald Jungle - Uses [Fruit Toss]",
        "health": 70,  "exp": 150, "attack_damage": 7,
        "notice_radius": 180, "attack_radius": 80,
    },
    {
        "name": "Grumpy Hawk",
        "sprite_name": "grumpy_hawk",
        "spawn": (12, 10),  
        "waypoints": [(12, 10), (20, 9)], 
        "patrol_type": "back_and_forth", 
        "speed": 1.5, 
        "description": "Cloudy Heights - Uses [Wind Gust]",
        "health": 85,  "exp": 150, "attack_damage": 12,
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, name, start_x, start_y, patrol_path, obstacle_sprites,
        ...
        self.is_dead = False
        self.respawn_delay = 10000  # 10초 (밀리초 단위)
        self.death_time = 0
    ...
    def animate(self):
        ...
        # 죽은 상태면 매 프레임 투명 유지 (animate()가 이미지를 교체하므로 여기서 처리)
        if self.is_dead:
    ...
    def get_damage(self, player):
        """플레이어 공격을 받아 데미지 처리. kill() 호출 없이 is_dead 플래그로만 관리."""
        if self.is_dead or not self.vulnerable:
            return
        ...
        if self.health <= 0:
            ...
            if hasattr(player, 'level'):
                player.level.team_kills += 1

            if player.is_local and hasattr(player, 'network') and player.network:
                team_data = {
                    "type": "team_score_update",
                    "team": getattr(player, 'team', 'None'),
                    "points": 1
                }
                player.network.send_update(team_data, self.rect.x, self.rect.y)
            # ✅ self.kill() 제거 — 스프라이트 그룹에서 빠지면 update()가 안 불려서 리스폰 불가
    ...
    def check_death(self):
        ...
        if self.health <= 0 and not self.is_dead:
            ...
            # set_alpha는 animate()에서 매 프레임 처리하므로 여기선 불필요

    def respawn(self):
        """에너미를 초기화하여 다시 살림"""
        self.is_dead = False
        self.health = self.max_health
        # ✅ set_alpha 제거 — animate()가 is_dead=False 확인 후 정상 렌더링함
