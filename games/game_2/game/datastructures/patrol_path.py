...
class PatrolPath:
    ...
    def add_waypoint(self, x, y, wait_time=0):
        ...
        # for creating the first element on the path
        if self.is_empty():
            self.head = waypoint
            self.tail = waypoint
            self.current = waypoint
        # for adding to the end of the patrol path
        else:
            self.tail.next = waypoint
            waypoint.prev = self.tail
            self.tail = waypoint
    ...
    def get_next_waypoint(self):
        ...
        Returns:
            Waypoint: The next waypoint to move toward, or None if patrol is complete
        """
        # check if a valid current list
        if self.is_empty() or self.current is None:
            return None
            
        # temporarily store to return later
        result = self.current
        
        # navigate list by traversal type
        if self.patrol_type == "one_way":
            self.current = self.current.next
        elif self.patrol_type == "circular":
            self.current = self.current.next
