...
# Module-level camera class (used by Level)
...
class Level:
    ...
    def create_enemies(self):
        """Create enemies — patrol types use linked list paths (Lab 5), random type
        wanders freely."""
        try:
            ...
            for data in ENEMY_SPAWN_DATA:
                try:
                    ...
                    exp=data.get("exp", 30),
                    score=data.get("score", 100), # for leaderboard
    ...
    def player_attack_logic(self):
        ...
        for attack_sprite in list(self.attack_sprites):
            for enemy in pygame.sprite.spritecollide(attack_sprite,
                ...
                if was_alive and enemy.health <= 0:
                    ...
                    self.player.add_score(enemy.score) # add to score for leaderboard
    ...
    def draw_names(self):
        ...
        for other_player in self.other_players.values():
            name_surface = self.font.render(other_player.name, True, (100, 100,
            255))
            name_rect = name_surface.get_rect(
    ...
    def draw_status(self):
        ...
        self.display_surface.blit(
        self.font.render("I: Inventory | SPACE: Attack", True, (255, 255,
        255)), (10, 40))
        ...
        self.display_surface.blit(self.font.render(f"HP {self.player.hp}/{self.player.max_hp}", True, (255, 255, 255)), (10 + HEALTH_BAR_WIDTH + 8, 70))

        # XP
        self.display_surface.blit(
        self.font.render(f"XP: {self.player.exp}", True, (255, 215, 0)), (10,
        100))
        # Equipped weapon
    ...
    def _snapshot_enemies(self):
        """Capture full enemy state (position, patrol cursor, combat) for time-
        travel."""
        enemies = []
        for enemy in self.enemies:
            enemies.append({
                ...
                'patrol_current': enemy.patrol_path.current if
                enemy.patrol_path else None,
                'patrol_direction': enemy.patrol_path.direction if
                enemy.patrol_path else None,
    ...
    def draw_time_travel_ui(self):
        ...
        if not self.connected:
            ...
            text = font_small.render("Time travel disabled (multiplayer)", True,
            (150, 150, 150))
            self.display_surface.blit(text, (10, 100))
